                          ...
                          if (
                              ...
                              component.componentText.forEach((row, rowIndex) => {
                                  if (rowIndex > 0) {
                                      ...
                                      console.log("COMPUTED", this.BMX_REPORT[this.categoryCounter - 1]['category_' + this.categoryCounter])
                                  }
                              });
                              //🌈🌈🌈🌈🌈🌈🌈🌈🌈🌈🌈🌈🌈🌈🌈🌈 USERS RESULTS
  ...
  computeReport(row, templateComponent, username, REPORT_DATA, reportType) {
      ...
      if (templateComponent.componentType == 'rate-scale' || templateComponent.componentType == 'image-rate-scale' ||
          ...
          if (templateComponent.componentSettings[0].CRITERIA) {
              ...
              const _comment = (row.Comments1?.length > 0) ? { userName: username, comment: row.Comments1 } : undefined;
              if (_comment) {
  ...
  createReportPerCategory() {
      ...
      this.categorySortedgArray.forEach((category, categoryIndex) => {
          ...
          category.forEach((row, rowIndex) => {
              ...
              row.comments = (this.commentsList[nameCandidates] || []).map(comment => (
                  `<div style="color: #ee7f25;">` + comment.userName + `: ` +
                  `<span style="color: #324395;">` + comment.comment + `</span>` + `</div>`
              )).join("\n").toString();
